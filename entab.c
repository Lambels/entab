...
int main(int argc, char *argv[]) {
    ...
    // split the stream in columns/tab stops. A tab is whitespace up to a tab stop.
    int i = 0;
    int inspace = 0;
    int startspace = 0;
    while ((ch = fgetc(instream)) != EOF) {
        if (i % t == 0) i = 0; // we are at a tabstop.
        
        if (i == 0 && inspace) fputc('\t', outstream);
        ...
        switch (ch) {
            ...
            default:
                if (inspace) {
                   inspace = 0;
                   for (int j = 0; j < (i - startspace); j++) fputc(' ', outstream);
                }
                fputc(ch, outstream);
                break;
        }        
    ...
    if (inspace) for (int j = 0; j < (i - startspace); j++) fputc(' ', outstream);
